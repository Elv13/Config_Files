...
class QListPrinter:
    ...
    class _iterator(Iterator):
        ...
        def __next__(self):
            curmode =  base2.current_print_mode()

            if curmode == "short" or self.count >= self.d['end'] - self.d['begin']:
                raise StopIteration
    ...
    def to_string(self):
        ...
        elemcount = self.val['d']['end']

        if elemcount == self.val['d']['begin']:
            return "[empty]"
        elif curmode == "short":
            return "["+str(elemcount)+" elems]"

        #TODO check if the value type has a display_hint == "string" or void?
        return "%s<%s>" % ( self.container, self.itype )
